...
RSpec.describe Notifications::Contracts::Refused::Supplier, type: [:service, :notification] do
  ...
  describe '.call' do
    ...
    describe 'notification' do
      ...
      describe 'admin notification' do
        ...
        it { expect(notification.action).to eq 'contract.refused_supplier' }
        ...
        describe 'args' do
          ...
          it { expect(notification.body_args).to eq [bidding.title, contract.proposal.provider.name] }
        end

        describe 'I18n' do
          let(:title_msg) { 'Contrato recusado pelo fornecedor.' }
          let(:body_msg) do
            "O contrato da licitação <strong>#{bidding.title}</strong> foi recusado pelo "\
            "fornecedor <strong>#{contract.proposal.provider.name}</strong>."
          end
