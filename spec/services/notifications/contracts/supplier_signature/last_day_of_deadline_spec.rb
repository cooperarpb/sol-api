...
RSpec.describe Notifications::Contracts::SupplierSignature::LastDayOfDeadline, type: [:service, :notification] do
  ...
  describe '.call' do
    ...
    describe 'notification' do
      ...
      describe 'suppliers notification' do
        ...
        it { expect(notification.action).to eq 'contract.supplier_signature_last_day_of_deadline' }
        ...
        describe 'I18n' do
          let(:title_msg) { 'Prazo encerra hoje!' }
          let(:body_msg) do
            "O prazo para assinatura do contrato "\
            "<strong>#{contract.title}</strong> encerra <strong>hoje</strong>."
          end
